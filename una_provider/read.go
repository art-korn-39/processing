...
func Read(db *sqlx.DB) {
	...
	stat := querrys.Stat_Select_una_provider()
	...
	logs.Add(logs.INFO, fmt.Sprintf("Чтение UNA провайдеров: %v [%s строк]", util.FormatDuration(time.Since(start_time)), util.FormatInt(len(data))))
