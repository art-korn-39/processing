...
func Read(db *sqlx.DB) {
	...
	for _, balance := range slice_balances {
		...
		hash := fmt.Sprint(balance.Provider_id, balance.Merchant_account_id, balance.Balance_currency.Name)
	...
	logs.Add(logs.INFO, fmt.Sprintf("Чтение балансов провайдеров из Postgres: %v [%s строк]", time.Since(start_time), util.FormatInt(len(data_maid)/2)))

}

func GetBalance(provider_id, ma_id int, balance_currency string) (*Balance, bool) {
	hash := fmt.Sprint(provider_id, ma_id, balance_currency)
	b, ok := data_maid[hash]
	return b, ok
}
