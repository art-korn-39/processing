...
func Write_CSV_SummaryProvider(s []SummaryRowProvider) {
	...
	headers := []string{
		...
		"count_operations",
		"channel_currency", "channel_amount", "BR_channel_currency",
		"balance_currency", "balance_amount", "BR_balance_currency",
	}
	...
	for _, v := range s {
		row := []string{
			...
			strings.ReplaceAll(fmt.Sprintf("%.2f", v.BR_channel_currency), ".", ","),
			v.Balance_currency_str,
			strings.ReplaceAll(fmt.Sprintf("%.2f", v.Balance_amount), ".", ","),
			strings.ReplaceAll(fmt.Sprintf("%.2f", v.BR_balance_currency), ".", ","),
		}
		writer.Write(row)
	}
...
func PSQL_Insert_SummaryProvider(s []SummaryRowProvider) {
	...
	channel := make(chan []SummaryRowProvider, 100)
